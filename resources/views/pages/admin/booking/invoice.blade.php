...
<div class="row">
    <div class="col-lg-12">
        <button class="btn btn-primary" id="save-pdf-button">Save PDF</button>
    </div>
   
    <div class="col-lg-12" id="content-pdf">
        <div class="card mt-3">
            <div class="card-header">Booking Invoice<strong>
                {{$data[0]->created_at}}
            </strong> <span class="float-end">
                    <strong>Status Booking :</strong> {{$data[0]->status_text}} </span> 
                    @if ($data[0]->is_paid==1)
            ...
            <div class="card-body">
                <div class="row mb-5">
                    <div class="mt-4 px-5 col-xl-3 col-lg-3 col-md-6 col-sm-12">
                        {{-- <h6>Customer:</h6> --}}
                        <div> <strong>{{$data[0]->name_customer}}</strong> </div>
                        {{-- <div>Madalinskiego 8</div>
                        <div>71-101 Szczecin, Poland</div> --}}
                        <div>Booking No. {{ $param1 }} </div>
                        <div>Booking Date  {{$data[0]->booking_date}} </div>
                    ...
                    <div class="mt-8 col-xl-9 col-lg-9 col-md-12 col-sm-12 d-flex justify-content-lg-end justify-content-md-center justify-content-xs-start">
                    ...
                    <table class="table table-border">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="left strong">{{ $item->service_name }}</td>
                            ...
                            <tr>
                                @php
                                $priceProduct += $item->total_price;
                                @endphp
                                {{-- <td class="center">{{ $loop->iteration }}</td> --}}
                                <td class="left strong">{{ $item->name_product }}</td>
                                <td class="right">{{ $item->price }}</td>
                        ...
                        <table class="table table-clear">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="right"><strong>Rp. {{ ($priceService+$priceProduct) }}</strong><br>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.0/html2canvas.min.js"></script>
    ...
    <script>
     ...
     $(document).ready(function() {
        ...
        // Ambil elemen HTML yang akan diubah menjadi PDF
        var element = $('#content-pdf')[0];

        // Konversi elemen HTML menjadi gambar menggunakan html2canvas
        html2canvas(element, { scale: 2 }).then(function(canvas) {
          var imageData = canvas.toDataURL('image/png');

          // Definisikan konten PDF dengan gambar HTML yang sudah dikonversi
          var content = [
            { image: imageData, width: 800, alignment: 'center' }
          ];
          ...
          var docDefinition = {
            ...
            pageSize: {
            width: 800, // Lebar kertas dalam satuan piksel
            height: 700 // Tinggi kertas dalam satuan piksel
            },
            margin: [20, 40, 20, 40]
          };

          // Generate PDF dan tampilkan dalam browser
